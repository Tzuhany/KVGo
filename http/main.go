...
func handleDelete(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil && !errors.Is(err, kvgo.ErrKeyIsEmpty) {
		http.Error(writer, err.Error(), http.StatusInternalServerError)
		log.Printf("failed to get kv in db: %v\n", err)
		return
	}
...
func main() {
	...
	// 注册 http 的接口
	http.HandleFunc("/kvgo/put", handlePut)
	http.HandleFunc("/kvgo/get", handleGet)
	http.HandleFunc("/kvgo/delete", handleDelete)
	http.HandleFunc("/kvgo/stat", handleStat)
	http.HandleFunc("/kvgo/listkeys", handleListKeys)
